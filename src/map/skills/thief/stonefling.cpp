...
void SkillStoneFling::applyAdditionalEffects(block_list *src, block_list *target, uint16 skill_lv, t_tick tick, int32 attack_type, enum damage_lv dmg_lv) const {
	if (!sc_start(src, target, SC_STUN, (2 * skill_lv - 1), skill_lv, skill_get_time(getSkillId(), skill_lv)))
		sc_start(src, target, SC_BLIND, (2 * skill_lv - 1), skill_lv, skill_get_time2(getSkillId(), skill_lv));
	...
	if (sd != nullptr) {
		...
		sc_start(src, target, SC_STUN, (2 * skill_lv - 1), skill_lv, skill_get_time(getSkillId(), skill_lv));
	}*/
}
